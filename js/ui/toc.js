// Toc rail. Walks the rendered document for top-level/section/subsection
...
const HEADING_SELECTOR = ".document__body :is(h1, h2, h3), .file-segment__title";

export class Toc extends Component {
    constructor(rootEl, deps) {
        super(rootEl, deps);
        this.textEl = document.getElementById("document-text");
     ...
     *
     *  We use a window scroll listener with sentinel rects (NOT the heading's
     *  own getBoundingClientRect) because the headings live inside
     *  `position: sticky` blocks: their painted rect is the stuck position,
     *  not the natural one, which makes IO-based tracking unreliable. The
     *  sentinel is in-flow, so its rect is always the heading's true
    ...
    recomputeActive() {
        ...
        // as in document.css. If the chrome height token changes, both
