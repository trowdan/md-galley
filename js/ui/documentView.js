// Document view component. Renders the loaded file and resolves selections
// into SELECTION_MADE payloads. Owns the text root inside the .document grid.
...
export class DocumentView extends Component {
     ...
     *  file currentFile tracking with a continuous flag. */
    onWorkspaceRendered({ files }) {
        const { html } = renderWorkspace(files);
        this.textEl.innerHTML = `<div class="document__body document__body--continuous">${html}</div>`;
    ...
    renderEmptyWorkspace(workspaceName) {
        ...
        this.textEl.innerHTML = `
            <div class="document__empty">
                <p class="document__empty-kicker">empty workspace</p>
                <h2 class="document__empty-title">No markdown files in ${name}.</h2>
                <p class="document__empty-body">
                    MDGalley walked this folder and found no <code>.md</code> files
    ...
    onFileLoaded({ filePath, source, documentTitle }) {
        const { html, data } = renderMarkdown(source);
        const titleHtml = renderMasthead(documentTitle ?? data?.title ?? filePath);
        this.textEl.innerHTML = `${titleHtml}<div class="document__body">${html}</div>`;
    ...
    emitSelection() {
        ...
        const documentTitle = anchor.filePath
            ? null
            : this.currentFile?.documentTitle ?? null;
    ...
    onClick(ev) {
        ...
        if (id) this.deps.bus.emit(Events.ANNOTATION_FOCUSED, { id, fromDocument: true });
    }
}

function renderMasthead(title) {
    return `
        <header class="document__masthead">
            <h1 class="document__title">${escape(title ?? "")}</h1>
        </header>
    `;
}
