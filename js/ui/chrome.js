...
export class Chrome extends Component {
    ...
    onFileLoaded({ documentTitle, filePath }) {
        const filename = filePath?.split("/").pop() ?? "";
        this.fileEl.textContent = filename || "no file open";
        document.title = documentTitle ? `${documentTitle} · MDGalley` : "MDGalley";
    }
    ...
    /** Flip continuous mode (single-document-across-files) and reload. The
     *  flag is read at workspace-open time, so a reload is the cheapest way
     *  to rebuild the rendered DOM without a runtime mode-switch path. */
    toggleContinuous() {
        let next = true;
        try {
        ...
        this.deps.bus.emit(Events.TOAST, {
            message: next ? "continuous mode: ON (reloading)" : "single-file mode: ON (reloading)",
        });
