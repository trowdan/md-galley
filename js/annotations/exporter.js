...
//   1. Every value that traces back to user-controlled text (documentTitle,
...
export const REVIEW_SCHEMA_VERSION = "mdgalley-review/2.0";
...
export function buildReviewMarkdown(byFile, meta = {}) {
    ...
    lines.push("");
    lines.push(`Scope values: \`anchored\` (passage), \`section\` (under a heading), \`document\` (whole file). Status: \`open\` notes need action, \`resolved\` notes are recorded for history.`);
    lines.push("");
    ...
    for (const [filePath, notes] of filesWithNotes) {
        ...
        const title = sorted[0].documentTitle;
        if (title) {
            lines.push("");
            lines.push(`**Document title (informational):**`);
            lines.push("```text");
...
function describeAnchor(note) {
    if (note.scope === "document") return "whole document";
 ...
 *  silently violating the schema's `review_date` type. */
